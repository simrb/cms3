<?php if ($t['_v'] == 'show') { ?>
					...
					if ($t["file_res"]) {
						while($row = mysql_fetch_array($t["file_res"])) {
							...
							echo "<td>" . $row['uid'] . "</td>";
	  						...
	  						echo "<td><a href='?_r=admin_file_del&fid=".
			...
			<?php
				if ($t["pagenums"] > 0) {
					for ($i=0; $i < $t["pagenums"]; $i++) {
						$j = $i + 1;
						echo "<span> <a href='?_r=admin_file&pagecurr=$j&".$t["url"];
						echo "'>$j</a> </span>";
					}
				}
	...
	<div class="show-bar">
		<form class="edit-form">
			<a href="?_r=admin_file__edit"><input type="button" value="<?= l('add'); ?>" class="bgwt" /></a>
		</form>
	</div>
...
<?php if ($t['_v'] == 'edit') { ?>
	...
	<div class="edit-form">
		...
		<form name="upload" action="?_r=admin_file_add_edit" enctype="multipart/form-data" method="post">
			<ul>
